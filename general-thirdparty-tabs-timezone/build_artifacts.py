...
MINIO_PROTOTYPE_URL = "https://minio.yc345.tv/onionext/review-pkg-demos/general-thirdparty-tabs-timezone-prototype-20260828-v3.html"
MINIO_PACKAGE_URL = "https://minio.yc345.tv/onionext/review-pkg-demos/general-thirdparty-tabs-timezone-review-20260825.html"
  ...
  <section class="main">
    ...
    <section id="filterArea" class="filters">
      ...
      <div class="filter-row">
        ...
        <div class="field"><label id="goodsNameLabel">洋葱商品名称</label><input id="goodsName" class="input" placeholder="请输入洋葱商品名称"></div>
        ...
        <div class="field"><label>退款状态</label><select id="refundStatus" class="select"><option value="">全部</option><option>已处理</option><option>未处理</option></select></div>
      </div>
      <div class="filter-row">
    ...
    <div id="orderTable" class="table-wrap">
      <table class="table">
        <colgroup><col style="width:180px"><col style="width:170px"><col style="width:150px"><col style="width:90px"><col style="width:90px"><col style="width:140px"><col style="width:110px"><col style="width:160px"><col style="width:155px"><col style="width:90px"><col style="width:100px"><col style="width:90px"><col style="width:130px"><col style="width:130px"><col style="width:100px"><col style="width:80px"></colgroup>
        <thead><tr><th id="goodsNameColumn">yc商品名称</th><th>yc订单ID</th><th>yc商品ID</th><th>yc订单实付金额</th><th>yc退款金额</th><th>yc用户ID</th><th>yc订单状态</th><th>父订单ID</th><th id="createdAtColumn">订单创建时间</th><th>支付金额</th><th>订单状态</th><th>退款金额</th><th>顺丰单号</th><th>普通单号</th><th id="channelColumn">店铺名称</th><th>操作</th></tr></thead>
        <tbody id="rows"></tbody>
      </table>
...
(() => {
const ecommerce=[
 {platform:'天猫',channelId:'499187844',channel:'洋葱学园旗舰店',goods:'小学数学同步课（12个月）',ycOrder:'6a8d127aab9c6ed83993a568',goodsId:'96cb56d2-8188-4cdf-863f-cb87939d246f',ycPaid:498,ycRefund:0,user:'668f51e82f81630001ce1c04',ycStatus:'支付成功',parent:'33163662390442438751',createdAt:'2026-08-25 11:56:40',paid:498,status:'支付成功',refund:0,confirmed:'已处理',sfTracking:'SF1510937202345',tracking:'—'},
 {platform:'小红书',channelId:'redbook-onion',channel:'洋葱学园课程店',goods:'7天全科同步课',ycOrder:'6a8d0fd68f333997f07474e3',goodsId:'4bfc6d78-f959-46b6-a6c5-7fddb9ca3b7d',ycPaid:19.9,ycRefund:0,user:'6690f21c3826f10001f32db9',ycStatus:'支付成功',parent:'P803181507530099321',createdAt:'2026-08-25 11:45:20',paid:19.9,status:'支付成功',refund:0,confirmed:'已处理',sfTracking:'—',tracking:'YT7826041956231'},
 {platform:'抖店',channelId:'douyin-yc',channel:'洋葱学园官方店',goods:'初中物理实验成长包',ycOrder:'6a8cfa206583dcd971268af2',goodsId:'2d331ea1-940c-4ebf-a390-721db0148850',ycPaid:299,ycRefund:299,user:'668d12f8da2b970001ce3cc9',ycStatus:'退款成功',parent:'DOU2026082400928',createdAt:'2026-08-24 20:08:11',paid:299,status:'退款成功',refund:299,confirmed:'未处理',sfTracking:'SF1510937202846',tracking:'—'}
];
const nonEcommerce=[
 {platform:'非电商平台的三方订单',channelId:'xiaobaobaba',channel:'小宝爸爸',goods:'小宝爸爸数学思维年度课',ycOrder:'6a910712f4ea593f142cc001',goodsId:'2f6b8c10-3f5d-4e2a-a942-8b77e1c39011',ycPaid:899,ycRefund:0,user:'6691aa7017a25e0001c055a8',ycStatus:'支付成功',parent:'xiaobaobaba20260818001',createdAt:'2026-08-18 15:26:42',paid:899,status:'支付成功',refund:0,confirmed:'已处理',sfTracking:'SF1510937203108',tracking:'—'},
 {platform:'非电商平台的三方订单',channelId:'davdian',channel:'大V店',goods:'大V店英语全年成长课',ycOrder:'6a910742178ab591cdf10122',goodsId:'6c1e927a-7d44-4fd6-b4a1-19d2a5348f22',ycPaid:699,ycRefund:0,user:'6691aba336ad770001b93a12',ycStatus:'支付成功',parent:'davdian20260818002',createdAt:'2026-08-18 15:42:08',paid:699,status:'支付成功',refund:0,confirmed:'已处理',sfTracking:'—',tracking:'JDVA00382619455'},
 {platform:'非电商平台的三方订单',channelId:'wanwuxinxuan',channel:'万物心选',goods:'万物心选物理实验课程包',ycOrder:'6a91078dcf01b993dd520163',goodsId:'9a53d870-2b16-4c99-a83f-67e45d1b7033',ycPaid:498,ycRefund:198,user:'6691ac1dd4f6830001c18b0a',ycStatus:'支付成功',parent:'wanwuxinxuan20260818003',createdAt:'2026-08-18 16:06:31',paid:498,status:'支付成功',refund:198,confirmed:'未处理',sfTracking:'SF1510937203672',tracking:'—'}
];
let tab='ecommerce';
...
function setup(){
 ...
 $('#goodsNameLabel').textContent=non?'商品名称':'洋葱商品名称';$('#goodsNameColumn').textContent=non?'商品名称':'yc商品名称';
 $('#channelId').placeholder=non?'请输入渠道ID':'请输入店铺id';$('#channelName').placeholder=non?'请输入渠道名称':'请输入店铺名称';
 $('#goodsName').placeholder=non?'请输入上传商品名称':'请输入洋葱商品名称';
}
function data(){
 ...
 return rows.filter(r=>(!f.p||r.platform===f.p)&&(!f.id||r.channelId.includes(f.id))&&(!f.name||r.channel.includes(f.name))&&(!f.parent||r.parent.includes(f.parent))&&(!f.goods||r.goods.includes(f.goods))&&(!f.goodsId||r.goodsId.includes(f.goodsId))&&(!f.user||r.user.includes(f.user))&&(!f.refund||r.confirmed===f.refund));
}
function render(){
 const rows=data();$('#rows').innerHTML=rows.map(r=>`<tr><td>${r.goods}</td><td class="mono">${r.ycOrder}</td><td class="mono">${r.goodsId}</td><td class="money">${r.ycPaid}</td><td class="money">${r.ycRefund}</td><td class="mono">${r.user}</td><td>${r.ycStatus}</td><td class="mono">${r.parent}</td><td class="mono">${r.createdAt}</td><td class="money">${r.paid}</td><td>${r.status}</td><td class="money">${r.refund}</td><td class="mono">${r.sfTracking}</td><td class="mono">${r.tracking}</td><td>${r.channel}</td><td><button class="link">激活课程</button></td></tr>`).join('');
...
{id:'chg-field-display',level:'module',page:'generalThirdpartyPage',module:'字段展示与筛选',target:'#orderTable',type:'logic',title:'非电商字段遵循三方订单表',desc:'非电商商品名称展示并筛选运营上传值；退款状态与线上一致；列表补充顺丰单号、普通单号。',before:'原型展示 yc 商品名称，退款选项与线上不一致，且缺少两个物流单号字段',after:'商品名称取 Excel 上传值；退款状态为全部/已处理/未处理；展示顺丰单号和普通单号',examples:[{case:'商品名称筛选',input:'大V店英语全年成长课',result:'只返回对应上传商品名称的订单'},{case:'退款状态',input:'未处理',result:'只返回未处理订单'}],sdd:{section:'FR-1 平台双 Tab 与查询隔离'}},
...
__ENGINE__
    ...
    "sdd:{section:'FR-1 平台双 Tab 与查询隔离',blockId:'doxcn5ZbuBWQ3Ewxzdcu02AKdQh'}",
).replace(
    "sdd:{section:'FR-2 时间筛选与展示统一'}",
    "sdd:{section:'FR-2 时间筛选与展示统一',blockId:'doxcnoAIP1MIHpvvuS8T0brbHqc'}",
).replace(
    "sdd:{section:'FR-3 导出口径'}",
    "sdd:{section:'FR-3 导出口径',blockId:'doxcnchalUqIp0J3B4tHT57Dhuc'}",
)
...
<header><h1>通用三方双 Tab 与时区统一 · 评审包</h1><div class="sub">高保真原型呈现平台数据分区、字段展示与筛选、订单创建时间一致性。</div><div class="meta"><span class="chip">需求类型：界面 · 中后台</span><span class="chip">通用三方</span><span class="chip">双 Tab</span><span class="chip">非电商平台</span><span class="chip">字段口径</span><span class="chip">created_at</span><span class="chip">Asia/Shanghai</span><span class="chip">导出</span></div></header>
<nav id="nav"><button class="on" data-tab="intro">① 简介</button><button data-tab="changes">② 变更清单</button><button data-tab="prototype">③ 高保真交互原型</button><button data-tab="decisions">④ 决策事项</button></nav>
<section id="intro" class="tab on">
<div class="card"><h3>待定问题</h3><table><tr><th>#</th><th>待定项</th><th>当前处理方式</th><th>需确认</th></tr><tr><td>1</td><td>上传商品名称是否与 yc 商品名称并列展示</td><td><span style="background-color:#FFF3CD;color:#856404;">【待定-1】非电商 Tab 暂以运营上传商品名称替换 yc 商品名称，不并列增加第二列</span></td><td>确认替换或两列并存</td></tr></table></div>
<div class="grid2"><div class="card"><h3>背景与问题</h3><ul><li><b>页面无平台类型分区：</b>通用三方现有电商数据与已 ready 的非电商数据缺少前端隔离入口。</li><li><b>非电商字段与既定口径不一致：</b>原型展示 yc 商品名称，退款状态选项与线上不同，且缺少顺丰单号、普通单号。</li><li><b>查询与展示字段错位：</b>筛选按订单创建时间，列表和导出却以 pay_time 充当主时间列并少 8 小时。</li></ul></div><div class="card"><h3>目标</h3><ol><li>默认电商平台 Tab，非电商订单只在非电商平台 Tab 展示，两类数据查询结果互不串行。</li><li>非电商页面字段展示与筛选遵循「三方订单表」口径：商品名称取运营上传值，退款状态与线上一致，展示两类物流单号。</li><li>列表查询、列表结果、导出文件中的订单创建时间保持一致。</li></ol></div></div>
<div class="grid2"><div class="card"><h3>范围 · 要做什么</h3><ul><li>新增「电商平台 / 非电商平台」双 Tab，视觉与掉单管理一致。</li><li>非电商 Tab 锁定第三方平台，店铺文案改为渠道 ID/名称。</li><li>非电商商品名称展示并筛选运营上传值；退款状态为「全部 / 已处理 / 未处理」。</li><li>列表补充「顺丰单号、普通单号」。</li><li>列表查询、列表结果、导出文件统一使用订单创建时间。</li></ul></div><div class="card"><h3>范围 · 不做什么</h3><ul><li>不展开 Excel 导入、掉单 Base、10MB 文件限制、上传渠道必填弹窗。</li><li>不改变现有导出的单平台与 90 天限制。</li><li>不新增主播、预售、买家留言、商品数量列。</li></ul></div></div>
<div class="card"><h3>相关文档</h3><table><tr><th>文档</th><th>链接</th></tr><tr><td>需求说明</td><td><a href="__REQUIREMENT_URL__" target="_blank">需求说明 · 通用三方双 Tab 与时区统一</a></td></tr><tr><td>SDD</td><td><a href="__SDD_URL__" target="_blank">SDD · 通用三方双 Tab 与时区统一</a></td></tr><tr><td>字段口径表</td><td><a href="https://guanghe.feishu.cn/sheets/shtcnEioTaphv14DdQVtmKlv6Eh" target="_blank">三方订单表 · 通用三方表</a></td></tr><tr><td>高保真原型</td><td><a href="__PROTOTYPE_URL__" target="_blank">外网原型</a></td></tr></table></div>
</section>
<section id="changes" class="tab"><h3>行为变更</h3><table><tr><th>序号</th><th>位置（端 · 平台 · 页面）</th><th>优先级</th><th>变更内容</th><th>开发团队</th></tr>
<tr><td>1</td><td>运营后台 · 通用三方列表</td><td>P0 / P1</td><td><b>1-1 P0 【新增】</b>双 Tab：电商平台默认、非电商平台独立展示。<br><b>1-2 P1 【变更】</b>非电商 Tab 锁定平台，店铺 ID/名称改为渠道 ID/名称。<br><b>1-3 P1 【变更】</b>查询按 Tab 隔离数据源，清空和切换不得串数据。<br><b>1-4 P1 【变更】</b>非电商商品名称展示并筛选运营上传值。<br><b>1-5 P1 【变更】</b>退款状态与线上一致：全部 / 已处理 / 未处理。<br><b>1-6 P1 【变更】</b>列表补充顺丰单号、普通单号。</td><td></td></tr>
...
decision_script = re.search(
    r'(<script>\s*/\*__DM_BOOT_START__\*/.*?</script>)',
    REVIEW_TEMPLATE,
    re.S,
).group(1)
...
| 相关文档 | 链接 |
|---|---|
| 需求评审包 | [内网评审包](""" + MINIO_PACKAGE_URL + """) |
| 对应 SDD | [SDD · 通用三方双 Tab 与时区统一](__SDD_URL__) |
| 高保真原型 | [内网高保真原型](""" + MINIO_PROTOTYPE_URL + """) |
